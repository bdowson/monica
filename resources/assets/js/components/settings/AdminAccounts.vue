...
<template>
  <div>
    ...
    <h3 class="mb3">
      {{ $t('settings.admin_accounts_title') }}
      <a class="btn nt2" :class="[ dirltr ? 'fr' : 'fl' ]" @click="showCreateModal">{{ $t('settings.admin_accounts_add') }}</a>
    </h3>
    <p>{{ $t('settings.admin_accounts_desc') }}</p>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getAccounts() {
                axios.get('/settings/admin/account-first-users')
                        .then(response => {
